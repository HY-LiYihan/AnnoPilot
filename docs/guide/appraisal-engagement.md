...
- **Review artifacts**：右侧运行状态可导出 Goldsmith/Rosetta-style `candidate_runs.jsonl`、`consistency_scores.jsonl`、`label_statistics.jsonl`、`contrastive_examples.jsonl`、`reflection_plans.jsonl`、`prompt_package.jsonl`、`verification_report.jsonl`、`human_review_queue.jsonl`、`human_choices.jsonl` 和 `hard_examples.jsonl`，把 engagement 标注中的候选、Rosetta route、不确定性分数、token-level entity/context/other 概率、similar/boundary examples、reflection checks、prompt tasks、verifier checks、candidate conflict risk、待审队列、人工选择、LLM review / judge、错配标记和边界失败样例交给离线优化/评估流程。
...
这个版本符合当前 1GB 内存目标：不引入向量库、队列或重型 optimizer；当前已把 Rosetta-style contrastive retrieval、reflection、prompt package 和 verifier report 作为导出型离线接口接入，后续再把更重的 prompt optimization runner 放到离线 pipeline。
