...
- **Run-level consistency**：每次 Character RAG 运行会不可变地保存所处理句子及完整 span 集合；`candidate_runs.jsonl` 按 run 输出整句候选，`consistency_scores.jsonl` 对最近 5 次完整 run 做 Rosetta-compatible span-set F1 / exact match 路由。同一次运行里多个并存的正确 Engagement cue 不再被误当成多个互斥候选。
