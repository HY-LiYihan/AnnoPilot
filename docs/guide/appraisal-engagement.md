...
- **Review artifacts**：右侧运行状态可导出 Goldsmith/Rosetta-style `candidate_runs.jsonl`、`consistency_scores.jsonl`、`label_statistics.jsonl`、`human_review_queue.jsonl`、`human_choices.jsonl` 和 `hard_examples.jsonl`，把 engagement 标注中的候选、Rosetta route、不确定性分数、token-level entity/context/other 概率、candidate conflict risk、待审队列、人工选择、LLM review / judge、错配标记和边界失败样例交给离线优化/评估流程。
