...
1. 在空白阅读器中选择一个内置样例：通用标注流程、新闻/政策叙事、学术/方法讨论、平台复核、客服反馈、合规/法律、社交舆情、财报/投资者沟通、医疗/科学传播、AI 教育、气候/能源转型、职场/劳动关系、产品安全/公众意见、危机回应/公共警示，或 Goldsmith/Rosetta 校准场景。
...
- **Review artifacts**：右侧运行状态可导出 Goldsmith/Rosetta-style `candidate_runs.jsonl`、`consistency_scores.jsonl`、`human_review_queue.jsonl`、`human_choices.jsonl` 和 `hard_examples.jsonl`，把 engagement 标注中的候选、Rosetta route、不确定性分数、candidate conflict risk、待审队列、人工选择、LLM review / judge、错配标记和边界失败样例交给离线优化/评估流程。
