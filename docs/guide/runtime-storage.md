...
- Span 可使用 Prodigy 的 `token_start` / `token_end`，也可使用 character offsets；两者同时存在时以 character offsets 为准，再映射到 AnnoPilot 本地 tokens，避免外部 tokenizer 的中英文分词差异把 span 映射到错误词。Prodigy offsets 按 sentence-local 解释，`annopilot.task.v1` offsets 按 document-global 解释，也可用 `offset_scope=sentence|document` 显式指定。
...
Product UI 使用 `summary` 读取全局 metrics、tags 和 sentence-dot state，使用 `sentences` 读取 paged reader window，使用 `review-queue` 读取右侧待审任务列表。Summary metrics 会输出 document-level `annotation_label_counts`、`annotation_overlap_count` 和 live pending `suggestion_label_counts`，sentence queue item 也会输出每句 `annotation_overlap_count` 供 UI 跳转到冲突句；同时输出 `suggestion_status_counts`、LLM latest review 的 `suggestion_review_counts` / `reviewed_suggestion_count`，并对当前可处理的 pending suggestions 输出 `suggestion_source_counts` 和 `suggestion_confidence_counts`，因此右侧运行状态无需加载全文 sentence window 也能展示 Engagement label coverage、span 边界冲突、待审队列质量和评审分布。Review queue 支持 `order=position`、`order=random`、`order=uncertain`、`order=goldsmith` 和 `order=hybrid`；`random` 提供稳定伪随机 baseline，`uncertain` 按句内最低 Character RAG confidence 优先，`goldsmith` 按词面低置信、句内候选密度、latest LLM review 风险、Rosetta-style judge 风险和候选标签/边界冲突综合优先，`hybrid` 保留高风险优先并插入少量高置信且无 LLM / judge 风险的 `calibration` 抽检样本。响应顶层 `rosetta_route_counts` 会按全量待审句子统计 low / medium / high consistency route，即使 UI 只加载前 20 条，也能显示完整队列的风险分布；右侧 readiness 入口会优先跳到当前加载队列里 `priority` 最高的待审项。队列 item 返回 `min_confidence`、`lexical_risk_score`、`llm_review_risk_score`、`judge_review_risk_score`、`candidate_disagreement_score`、`risk_score`、`risk_reason_codes`、`review_route`、Rosetta-compatible `rosetta_route`、人工复核 `priority` 和兼容字段 `priority_score=min_confidence`，其中 latest LLM review 为 `reject` / `uncertain` 的 pending suggestions 会提高 `llm_review_risk_score`，latest `judge` 中低 `overall_score` / `boundary_score`、高 `missed_span_risk` / `extra_span_risk`、`needs_review`、`error_types` 或 `risk_flags` 会提高 `judge_review_risk_score`，同句候选之间的 label 或 boundary 冲突会提高 `candidate_disagreement_score`。`rosetta_route=low|medium|high` 表示 Rosetta-style consistency route，`priority` 用 `low=100`、`medium=50`、`high=10` 的 route base 加不确定性生成，数值越高越适合优先人工复核。UI 在 `goldsmith` / `hybrid` 风险项显示综合 `risk_score`、risk breakdown、`priority` 和 `risk_reason_codes` 对应的原因 chips，在 `hybrid` 抽检项显示校准置信度。旧的 full document endpoint 仍保留，用于兼容和 export-adjacent workflows。
...
- Export manifest 记录 source run ids、annotation import history、schema version、artifact hashes、`prodigy_readiness`（含 verification status / issue counts）、`prodigy_labels_json`、event actor/type audit summary、timestamp 和稳定 content hash。
