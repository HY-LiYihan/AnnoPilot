...
- 支持先选择 token/span，再用数字键或左侧 tag 应用标签。
...
- 支持独立 review queue API 和右侧 review queue 列表；处理完当前句建议后会自动跳到下一句待确认，保留 `R` 快捷键手动跳转。
...
当前 `backend/app/storage.py` 使用 SQLite 作为 runtime store，使用 JSONL event log 作为 durable audit trail。
右侧 Accuracy 指标当前定义为 LLM review recommendation 与已执行 accept/reject 动作的一致率；没有已 review 且已决策的样本时显示等待数据，不伪造 gold accuracy。
Character RAG 会使用已确认 annotations 作为正例，并把项目内 rejected suggestions 的 `tag_id + text` 作为负例，后续生成建议时跳过同样的词面/标签组合；匹配判断会对空白和大小写做归一化，因此英文大小写变体不会重复打扰 review 队列。
...
Run history 会展示每次 Character RAG run 的 pending / accepted / rejected counts 和基于已决策样本的 acceptance rate，便于判断自动建议队列质量。每个 run 可导出 provenance JSON，追踪 config、当次正例/负例词表快照、归一化 match keys、evidence、LLM review 和 accept/reject decision event。
