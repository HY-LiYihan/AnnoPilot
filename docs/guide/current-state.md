...
- 支持独立 review queue API 和右侧 review queue 列表，可按原文位置或低 confidence 优先排序；summary metrics 会直接展示建议状态、LLM 评审推荐分布、待审建议来源和置信度分布；处理完当前句建议后会自动跳到下一句待确认，保留 `R` 快捷键手动跳转。
