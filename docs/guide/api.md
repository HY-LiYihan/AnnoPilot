...
- `sample-presets` 返回后端内置的轻量样例索引；当前包含通用、新闻/政策、学术/方法、平台复核、客服反馈、合规/法律、社交舆情、财报/投资者沟通、医疗/科学传播、AI 教育、气候/能源、职场/劳动关系、产品安全/公众意见，以及 Goldsmith/Rosetta 校准场景十四类 bilingual Engagement 样例。每个 preset 还返回 `auto_accept_on_load` 和 `complete_sentences_on_load`，供 UI 按场景选择快速标注或保留 review 候选。
...
- Goldsmith review tasks JSONL 使用 `annopilot.goldsmith_review_tasks.v1`，把 low / medium consistency route 的同句候选聚合为 Rosetta-style `human_review_task`，包含中文 prompt、priority、按 `candidate_id` 稳定排序的候选选项 A/B/C、`manual_option_id=__manual__`、inline markup 和 consistency 摘要，直接服务人工复核队列。
