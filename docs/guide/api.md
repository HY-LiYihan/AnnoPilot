...
- `monogloss/auto-mark` 是显式确认的高风险自动化入口：默认返回 400，不会因为打开 readiness 面板或普通点击而批量写入 Monogloss。只有调用时显式传 `?confirm=true` 才会为未完成、无 annotation、无 pending suggestion 的句子创建整句 Monogloss span 并写入 `answer=accept`；正式人工流程建议使用当前句 `M` 后逐句检查。
