...
- Goldsmith review tasks JSONL 使用 `annopilot.goldsmith_review_tasks.v1`，把 low / medium consistency route 的同句候选聚合为 Rosetta-style `human_review_task`，包含中文 prompt、priority、候选选项 A/B/C、`manual_option_id=__manual__`、inline markup 和 consistency 摘要，直接服务人工复核队列。
