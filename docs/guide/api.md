...
- `review-queue` 返回未完成且存在 pending suggestions 的句子列表，以及每句第一条候选，供 UI 快速跳转待审任务；`order=uncertain` 会按最低 Character RAG confidence 优先排序，`order=goldsmith` 会按低置信度和句内候选密度的综合风险优先排序，`order=hybrid` 会保留高风险优先，同时插入少量高置信 `calibration` 抽检样本。队列 item 同时返回 `min_confidence`、`risk_score` 和 `review_route`，其中 `risk_score=(1-min_confidence)×suggestion_count`；旧字段 `priority_score` 保持为 `min_confidence` 以兼容已有调用。
...
- `sample-presets` 返回后端内置的轻量样例索引；当前包含通用、新闻/政策、学术/方法、平台复核四类 bilingual Engagement 样例。
