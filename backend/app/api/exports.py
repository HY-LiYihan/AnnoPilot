...
@router.get("/documents/{document_id}/export.goldsmith.prompt-package.jsonl")
def export_goldsmith_prompt_package(
    project_id: str,
    document_id: str,
    storage: AnnotationStorage = Depends(get_storage),
) -> StreamingResponse:
    try:
        lines = storage.export_goldsmith_prompt_package_lines(project_id, document_id)
    except NotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    headers = {"Content-Disposition": f'attachment; filename="{document_id}.goldsmith.prompt-package.jsonl"'}
    return StreamingResponse(iter(lines), media_type="application/x-ndjson", headers=headers)
...
def export_goldsmith_review_tasks(
    ...
    try:
        lines = storage.export_goldsmith_review_task_lines(project_id, document_id)
    except NotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    headers = {"Content-Disposition": f'attachment; filename="{document_id}.goldsmith.review-tasks.jsonl"'}
    return StreamingResponse(iter(lines), media_type="application/x-ndjson", headers=headers)


@router.get("/documents/{document_id}/export.goldsmith.verification-report.jsonl")
def export_goldsmith_verification_report(
    project_id: str,
    document_id: str,
    storage: AnnotationStorage = Depends(get_storage),
) -> StreamingResponse:
    try:
        lines = storage.export_goldsmith_verification_report_lines(project_id, document_id)
    except NotFoundError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    headers = {"Content-Disposition": f'attachment; filename="{document_id}.goldsmith.verification-report.jsonl"'}
    return StreamingResponse(iter(lines), media_type="application/x-ndjson", headers=headers)
